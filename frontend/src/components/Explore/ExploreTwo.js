...
class SellAssetCmp extends Component {
    ...
    loadCollections = async() => {
        ...
        let items = await Promise.all(collections.map( async(collection) => {
            ...
            let citem = {
                ...
                currentMints: curCount.toBigInt(),
                totalMints: collection.totalMints.toBigInt(),
                limits: collection.limits.toBigInt()
            }
            return citem;
        }));
    ...
    loadNFTs = async(collectionId) => {
        ...
        const items = await Promise.all(data.map(async i => {
            ...
            let item = {
                collectionId: i.collectionId.toBigInt(),
                name: cinfo.name,
                itemId: i.itemId.toBigInt(),
                tokenId: i.tokenId.toBigInt(),
    ...
    onBuyNFT = async(itemInfo, idx) => {
        ...
        console.log("buyingFee:", buyingFee.toBigInt());
        console.log("buyingPrice:", buyingPrice.toBigInt());
        ...
        console.log(buyingPrice.toBigInt()+","+buyingFee.toBigInt());
        let transaction = await contractCoin.approve(nftmarketaddress, buyingPrice.toBigInt());
        ...
        transaction = await contractCoin.approve(nftmarketaddress, buyingFee.toBigInt());
        tx = await transaction.wait();
        console.log("transaction for approving:", tx);
                                        ...
                                        <div className="card border-effect ap_anim">
                                            ...
                                            <div className="card-caption col-12 p-0">
                                                ...
                                                { (this.state.startProgress && this.state.selectItemId == idx)?
                                                    ...
                                                    <div style={{display:"flex", justifyContent:"center"}}>
                                                        <span style={{marginLeft:"20px"}}>{item.price.div(ethers.BigNumber.from(Math.pow(10,9))).toBigInt()} ADMC</span>
                                                    </div>
                                                    <div style={{display:"flex", justifyContent:"center"}}> 
