...
class Create extends Component {
    ...
    onCreateCollection = async() => {
        ...
        // if( String(owner).toString().toLowerCase() !== localStorage.getItem('select-address').toLowerCase()){
        //     alert('owner can only access here '); return;
        // }
        ...
        let transaction = await contract.addCollection(
            this.state.metadata.name, this.state.metadata.url, 100000, 1000, 100000
        );
