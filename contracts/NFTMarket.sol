...
contract NFTMarket is ReentrancyGuard{
    ...
    address m_addrOwner = address(0x1FE35381E6fB8580b05bc297f82fD0da01CF75bd);
    ...
    uint256 m_nDecimal = 18;
    uint256 private m_nListingPrice = 100000 * 10**m_nDecimal; // ADMC Coin
    ...
    function createMarketItem(
        ...
        payWithADMC(nftContract, m_addrOwner, m_nListingPrice);
        _itemIds.increment();
    ...
    function sendToWallet(address nftContract, address to, uint256 costs) public {
        address addressCoin = NFT(nftContract).getPayableCoin();
        IERC20 admc = IERC20(addressCoin);
        require(admc.balanceOf(address(this)) >= costs, 
                "contract balance is too small, you can't pay for minting.");
        admc.transfer(to, costs);
    }
    ...
    function payBuyingFeeToAdmin(
        ...
        payWithADMC(nftContract, m_addrOwner, fee); // pay fee to owner with adamant coin.
    }
    ...
    function createMarketSale (
        ...
        idToMarketItem[itemId].price = price * 10**m_nDecimal;
    ...
    function setListingPrice(uint256 price) public onlyByOwner nonReentrant{
        m_nListingPrice = price * 10**m_nDecimal;
    }
